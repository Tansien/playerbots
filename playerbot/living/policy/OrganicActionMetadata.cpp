    ...
    namespace
    {
        ...
        constexpr OrganicActionMetadataTable METADATA = { {
            ...
            { OrganicActionKind::MONEY_PERIODIC, "MONEY_PERIODIC", OrganicActionCategory::Wealth,
              "RandomPlayerbotMgr::Refresh money grant; Refresh is a shared boundary also reached from teleport paths, BattleGroundJoinAction and LfgActions, and additionally revives/repairs/refills/fabricates", OrganicClassification::Deny,
              nullptr, false, false, "0002A 'Starting/periodic money'" },
            { OrganicActionKind::TEMP_MONEY_TRICK, "TEMP_MONEY_TRICK", OrganicActionCategory::Wealth,
              "shared temporary-SetMoney boundary: RpgSubActions flight, WorldBuffTravelActions flight, MovementActions taxi/gold-cheat fare, SellAction gold-cheat sell-then-restore, buy/repair helpers (SetMoney runs MoneyChanged synchronously, which can permanently auto-reward money-required quests; restoring the balance cannot undo that)", OrganicClassification::Deny,
            ...
            { OrganicActionKind::DIRECT_ITEM_DESTRUCTION, "DIRECT_ITEM_DESTRUCTION", OrganicActionCategory::Inventory,
              "SmartDestroyItemAction/DestroyItemAction direct discard from maintenance and bag-pressure paths", OrganicClassification::Deny,
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row (core destroy-item handler for any allowed automation)" },
            { OrganicActionKind::LOGIN_ITEM_FABRICATION, "LOGIN_ITEM_FABRICATION", OrganicActionCategory::Inventory,
              "PlayerbotHolder::OnBotLogin: StoreNewItemInBestSlots(6948/40582) whenever hearthstone/death gate is missing", OrganicClassification::Deny,
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row (starter items only at managed bootstrap)" },
            ...
            { OrganicActionKind::FREE_TALENT_RESPEC, "FREE_TALENT_RESPEC", OrganicActionCategory::Progression,
              "ChangeTalentsAction / TalentSpec::ApplyTalents: removeSpell/learnSpell respec without cost or trainer", OrganicClassification::Deny,
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row (core respec rules only; earned-point spending stays automation)" },
            ...
            { OrganicActionKind::GAMEPLAY_QUEST_ACCEPT, "GAMEPLAY_QUEST_ACCEPT", OrganicActionCategory::Quests,
              "core-eligible AddQuest through the normal quest-giver/shared/item handlers; QuestAction::AcceptQuest also carries the denied SyncQuestWithPlayer fallback, so the guard must bind to that branch, not the action entry", OrganicClassification::AllowGameplay,
              nullptr, true, false, "0002A 'Normal quest reward/loot/equip' family; positive counterpart of REMOTE_QUEST_ACCEPT" },
            { OrganicActionKind::REMOTE_QUEST_ACCEPT, "REMOTE_QUEST_ACCEPT", OrganicActionCategory::Quests,
              ...
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row (quests accepted through normal handlers only)" },
            { OrganicActionKind::DIRECT_QUEST_ABANDON, "DIRECT_QUEST_ABANDON", OrganicActionCategory::Quests,
              "PlayerbotAI::DropQuest callers: CleanQuestLogAction/DropQuestAction pruning, QuestUpdateFailedTimerAction, GuildAcceptQuestOrderAction", OrganicClassification::Deny,
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row (core abandon semantics only)" },
            ...
            { OrganicActionKind::ENCOUNTER_AURA_MUTATION, "ENCOUNTER_AURA_MUTATION", OrganicActionCategory::CharacterInit,
              "KarazhanDungeonActions (Netherspite): RemoveAurasDueToSpell on the boss beams and AddAura(30421) on the bot", OrganicClassification::Deny,
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row (encounter mechanics through core rules only)" },
            { OrganicActionKind::DIRECT_GLYPH_MUTATION, "DIRECT_GLYPH_MUTATION", OrganicActionCategory::CharacterInit,
              ...
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row (core glyph handler only)" },
            ...
            { OrganicActionKind::FREE_PET_HAPPINESS, "FREE_PET_HAPPINESS", OrganicActionCategory::CharacterInit,
              "FeedPetAction: SetPower(POWER_HAPPINESS) without consuming food or casting Feed Pet", OrganicClassification::Deny,
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row (normal feed item/spell path only)" },
            ...
            { OrganicActionKind::GUILD_BANK_TAB_MUTATION, "GUILD_BANK_TAB_MUTATION", OrganicActionCategory::Social,
              "BuyGuildBankTabAction: buys guild-bank tabs and sets rights without a guild-bank interaction", OrganicClassification::Deny,
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row" },
            ...
            { OrganicActionKind::UNOBSERVED_MOVE_TELEPORT, "UNOBSERVED_MOVE_TELEPORT", OrganicActionCategory::Relocation,
              "MovementActions activity-throttled TeleportTo paths (area-trigger node, long-distance, flight-path shortcuts) independent of EnableMinimalMove", OrganicClassification::Deny,
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row (normal movement only for managed bots)" },
            ...
            { OrganicActionKind::DIRECT_SUMMON_TELEPORT, "DIRECT_SUMMON_TELEPORT", OrganicActionCategory::Relocation,
              "SummonAction/UseMeetingStoneAction/WorldBuffTravelActions/CastCustomSpellAction helpers: direct SetSummonPoint/ResurrectPlayer(1.0)/TeleportTo without core summon rules", OrganicClassification::Deny,
            ...
            { OrganicActionKind::GAMEPLAY_DEATH_RECOVERY, "GAMEPLAY_DEATH_RECOVERY", OrganicActionCategory::Recovery,
              "corpse release and resurrection through the core handlers, which validate their own preconditions", OrganicClassification::AllowGameplay,
              nullptr, true, false, "0002A 'Corpse release/spirit healer/resurrection spell' (direct spirit-healer path is DIRECT_SPIRIT_HEALER_RESURRECTION)" },
            ...
            { OrganicActionKind::DEATH_RECOVERY_TELEPORT, "DEATH_RECOVERY_TELEPORT", OrganicActionCategory::Recovery,
              ...
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row (normal corpse run remains gameplay)" },
            { OrganicActionKind::DIRECT_SPIRIT_HEALER_RESURRECTION, "DIRECT_SPIRIT_HEALER_RESURRECTION", OrganicActionCategory::Recovery,
              "SpiritHealerAction: ResurrectPlayer/DurabilityLossAll/SpawnCorpseBones/SaveToDB even when no spirit healer was found (core handlers require an interactable healer)", OrganicClassification::Deny,
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row (core spirit-healer handler only)" },
            ...
            { OrganicActionKind::AUCTION_TRANSACTION, "AUCTION_TRANSACTION", OrganicActionCategory::Economy,
              "auctions through the core auction session handlers only (HandleAuctionPlaceBid etc., as in AhAction)", OrganicClassification::AllowAutomation,
              nullptr, true, false, "0002A 'AH/mail/trade' (direct-mutation path is DIRECT_AUCTION_MUTATION)" },
            { OrganicActionKind::DIRECT_AUCTION_MUTATION, "DIRECT_AUCTION_MUTATION", OrganicActionCategory::Economy,
              "GuildShareAhBuyAction: AuctionEntry::UpdateBid called directly, bypassing HandleAuctionPlaceBid", OrganicClassification::Deny,
              nullptr, false, false, "0002A A.2 Phase 0 source-audit row (core auction handlers only)" },
            ...
            { OrganicActionKind::BROAD_MAINTENANCE_COMMAND, "BROAD_MAINTENANCE_COMMAND", OrganicActionCategory::Maintenance,
              "broad init/refresh/upgrade/maintenance console and chat commands (wildcard '%' targeting: 0.1 call sites must preflight every target is FIXTURE; per-identity evaluation denies non-fixture targets)", OrganicClassification::FixtureOnly,
            ...
            { OrganicActionKind::DEBUG_MUTATION_COMMAND, "DEBUG_MUTATION_COMMAND", OrganicActionCategory::Maintenance,
              "DebugAction (cdebug) mutation subcommands (do/quest/position/npc/...) reachable by group members without moderator security", OrganicClassification::FixtureOnly,
              nullptr, false, true, "0002A A.2 Phase 0 source-audit row (read-only diagnostics excluded; 0.1 must gate mutation subcommands)" },
