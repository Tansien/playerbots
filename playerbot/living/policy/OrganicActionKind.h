...
namespace living
{
    ...
    // families guarded by the Organic policy.
    //
    // This inventory is explicitly NOT complete. It is seeded from the normative
    // compatibility matrix in docs/design/0002-organic-policy-compatibility-matrix.md
    // (0002A) plus the cheat-mask categories in PlayerbotAIConfig.h, and grows with
    // each Phase 0 source audit. Per the 0002A A.2 completion rule, proving that no
    // state-changing shortcut exists outside it (repository search, call-graph
    // review, runtime instrumentation) is a 0.1 acceptance gate, not a claim this
    // tranche makes; every newly found path gets its own enumerator and matrix row.
    //
