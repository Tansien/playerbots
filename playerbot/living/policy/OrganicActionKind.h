...
namespace living
{
    // Typed inventory of the synthetic-mutation, shortcut, and ordinary-gameplay
    // action families the Organic policy decides on.
    //
    // Seed provenance (docs/design/0002-organic-policy-compatibility-matrix.md,
    // A.2): the rows of the normative 0002A matrix plus module source paths found
    // by Phase 0 source audits. The inventory is explicitly NOT claimed complete:
    // proving no state-changing shortcut exists outside it is a 0.1 acceptance
    // gate. Every newly found path gets its own enumerator and metadata row; a
    // generic "misc synthetic action" escape value is prohibited, so an action
    // without an enumerator fails closed as unknown.
    //
    // Each enumerator has exactly one metadata row, in enum order, in
    // OrganicActionMetadata.cpp; a static_assert there fails the build on a
    // missing, misordered, or inconsistent row.
    enum class OrganicActionKind : uint16_t
    {
        ...
        // character-state initialization and direct state edits
        WORLD_BUFF_APPLY,
        SHARED_RESPAWN_ACCELERATION,
        DIRECT_AURA_REMOVAL,
        ...
        // the 13 configured cheat-mask categories (BotCheatMask) and the
        // per-bot runtime override
        CHEAT_TAXI,
        CHEAT_GOLD,
        CHEAT_HEALTH,
        CHEAT_MANA,
        ...
        // test fixtures (invariant 12: never Organic characters)
        FIXTURE_CHARACTER_CREATE,
        FIXTURE_PROVISION,
